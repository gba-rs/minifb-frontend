...
const THUMB_W: usize = 112;
const THUMB_H: usize = 75;
...
pub fn render(buf: &mut [u32], stride: usize, rect: &Rect, gba: &GBA) {
    ...
    for bg_number in 0..4usize {
        if !gpu.display_control.should_display(bg_number as u8) {
            continue;
        }
        if mode >= 3 && bg_number != 2 {
            continue;
        }
        ...
        let col = bg_number % 2;
        let row = bg_number / 2;
        let dest_x = rect.x + 4 + col * (THUMB_W + 4);
        let dest_y = rect.y + 18 + row * (THUMB_H + 18);

        font::draw_text(buf, stride, dest_x, dest_y - 10, &format!("BG{}", bg_number), draw::COL_DIM, 1);
        blit_scaled(buf, stride, &frame, dest_x, dest_y);
    }
}

fn decode_background(gpu: &GPU, mem_map: &MemoryMap, bg_number: usize, mode: u8) -> Vec<Rgb15> {
    if mode >= 3 {
...
fn decode_text_mode(gpu: &GPU, mem_map: &MemoryMap, bg_number: usize) -> Vec<Rgb15> {
...
fn decode_affine_text(gpu: &GPU, mem_map: &MemoryMap, bg_number: usize) -> Vec<Rgb15> {
    let mut frame = vec![Rgb15::new(0x8000); GBA_WIDTH * GBA_HEIGHT];
...
fn decode_bitmap(gpu: &GPU, mem_map: &MemoryMap, mode: u8) -> Vec<Rgb15> {
    let mut frame = vec![Rgb15::new(0x8000); GBA_WIDTH * GBA_HEIGHT];
...
fn blit_scaled(buf: &mut [u32], stride: usize, frame: &[Rgb15], dest_x: usize, dest_y: usize) {
